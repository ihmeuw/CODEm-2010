...
Purpose: 	Run the linear portion of the model with country random effects (for linear only model)
*/

// setup the program
    cap log close
    log using "${base_dir}/logs/2_linear_country_${submodel_chunk}_${test}.smcl", replace
	display "Running linear submodels ${start_submodel} through ${end_submodel} with country random effects for test ${test} on node `r(o1)'"
	timer on 1
	set more off
	set mem 2g
	set matsize 11000
    ...
    if $ref_cov == 1 {
        ...
        foreach c of local varlist {
            ...
            foreach pred_c of global pred_cov_list {
                ...
                else if substr("`c'",-3,.) == "_sq" & substr("`c'",1,3) != "ln_" & substr("`c'",1,3) != "lt_" & substr("`c'",.,(length("`c'")-3)) == "`pred_c'" {
                    local this_ref_name = word("$ref_cov_list",`count_word')
                    local pred_transforms `pred_transforms' `c'
...
// remove the model results file if it already exists
	capture rm "$temp_dir/2_model_estimates_${submodel_chunk}_${test}.ster"
...
// run the vanilla xtmixed model
		if "${type_`i'}" == "covariate" {
			...
			// run the regression
			xtmixed train_${dv_`i'} ${covariates_`i'} `age_dummy' || super_region: || region: || age: || iso3:, iterate(30)

           	// predict the linear and random effects components for subnational, to be used by residuals
			quietly {
				capture predict xb_`i'_sn, xb
				if _rc generate xb_`i'_sn = .
				capture predict u_s_`i'_sn u_r_`i'_sn u_a_`i'_sn u_c_`i'_sn, reffects
				if _rc {
					generate u_s_`i'_sn = .
					generate u_r_`i'_sn = .
					generate u_a_`i'_sn = .
				...
				bysort age iso3: egen u_c_`i'_mean_sn = mean(u_c_`i'_sn)
				replace u_c_`i'_sn = u_c_`i'_mean_sn if u_c_`i'_sn == .
				replace u_c_`i'_sn = 0 if u_c_`i'_sn == .
				// generate the final predictions
				generate linear_`i'_${test}_subnatl = xb_`i'_sn + u_s_`i'_sn + u_r_`i'_sn + u_a_`i'_sn + u_c_`i'_sn
				drop u_*_mean_sn
			}

           // save the model so we can derive uncertainty later
			estimates save "$temp_dir/2_model_estimates_${submodel_chunk}_${test}.ster", append
			...
			quietly {
				...
				capture predict u_s_`i' u_r_`i' u_a_`i' u_c_`i', reffects
				if _rc {
					generate u_s_`i' = .
					generate u_r_`i' = .
					generate u_a_`i' = .
				...
				bysort age iso3: egen u_c_`i'_mean = mean(u_c_`i')
				replace u_c_`i' = u_c_`i'_mean if u_c_`i' == .
				replace u_c_`i' = 0 if u_c_`i' == .
				// generate the final predictions
				generate linear_`i'_${test} = xb_`i' + u_s_`i' + u_r_`i' + u_a_`i' + u_c_`i'
				drop u_*_mean
			}
...
// save the first stage results for each super region
	...
	save "$temp_dir/2_linear_country_${submodel_chunk}_${test}.dta", replace
	restore

// save the fixed/random effects predictions for use later when determining uncertainty
	keep n u_* xb_*
	save "$temp_dir/2_model_effects_${submodel_chunk}_${test}.dta", replace
